 ...
 */
MilestoneSchema.statics.findUpcoming = function (daysAhead = 7): Promise<IMilestone[]> {
  const now = new Date();
  const cutoff = new Date(now.getTime() + daysAhead * 24 * 60 * 60 * 1000);
  return this.find({ dueDate: { $gte: now, $lte: cutoff } }).sort({ dueDate: 1 });
};

/**
 * Find milestones that are overdue
 */
MilestoneSchema.statics.findOverdue = function (): Promise<IMilestone[]> {
  return this.find({ dueDate: { $lt: new Date() } }).sort({ dueDate: 1 });
};

// --- Model Export ---
export const Milestone = mongoose.model<IMilestone, IMilestoneModel>("Milestone", MilestoneSchema);
